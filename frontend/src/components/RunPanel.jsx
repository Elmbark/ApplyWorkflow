...
export function RunPanel({ onRunComplete }) {
  ...
  return (
    <div style={s.card}>
      <div style={s.header}>
        <span style={s.title}>Run Pipeline</span>
      ...
      <div style={s.body}>
        <div style={s.field}>
          ...
          <input style={s.input} value={company} onChange={e => setCompany(e.target.value)}
            placeholder="Leave blank for all" disabled={running} />
        </div>
        <div style={s.field}>
          <label style={s.label}>Limit rows (0 = all)</label>
        ...
        <div style={s.checkRow}>
          ...
          <label htmlFor="chk-pdf" style={s.checkLabel}>Compile PDF (Typst)</label>
        </div>
        <div style={{ ...s.checkRow, marginBottom: 18 }}>
          ...
          <label htmlFor="chk-send" style={s.checkLabel}>Send emails via Gmail</label>
        </div>
        ...
        <div style={{ marginTop: 18 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
            <label style={s.label}>Live Output</label>
            <button style={s.smBtn} onClick={() => setLogs([])}>Clear</button>
          </div>
...
const s = {
  ...
  label: { display: 'block', fontSize: 12, fontWeight: 500, color: '#94a3b8', marginBottom: 5 },
  ...
  checkLabel: { fontSize: 13, color: '#e2e8f0', cursor: 'pointer' },
  ...
  smBtn: { fontSize: 12, padding: '3px 9px', background: 'rgba(255,255,255,.07)', border: '1px solid rgba(255,255,255,.1)', borderRadius: 5, color: '#94a3b8', cursor: 'pointer' },
